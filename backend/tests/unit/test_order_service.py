...
def setup_data(db_session):
    ...
    customer = c_service.create_customer(
        CustomerCreate(full_name="Order Tester", email="order@tester.com", phone="123")
    )

    product1 = p_service.create_product(
        ProductCreate(name="Item A", sku="ITM-A", price=10.0, quantity_in_stock=5)
    )

    product2 = p_service.create_product(
        ProductCreate(name="Item B", sku="ITM-B", price=20.0, quantity_in_stock=2)
    )
...
def test_create_order_insufficient_stock(db_session, setup_data):
    ...
    create_dto = OrderCreate(
        customer_id=setup_data["customer"].id,
        items=[OrderItemCreate(product_id=setup_data["p1"].id, quantity=6)],
    )
...
def test_concurrent_orders_race_condition(db_session, setup_data):
    ...
    # So we will just test the normal stock exhaustion, and we will
    # inspect the code for with_for_update. To be safe, we'll order exactly the remaining amount.
