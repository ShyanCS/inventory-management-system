...
def test_create_product(db_session):
    ...
    create_dto = ProductCreate(
        name="Test Product", sku="TEST-001", price=10.00, quantity_in_stock=5
    )
...
def test_create_product_duplicate_sku(db_session):
    service = ProductService(db_session)
    create_dto = ProductCreate(
        name="Test Product", sku="TEST-002", price=10.00, quantity_in_stock=5
    )
...
def test_list_low_stock_products(db_session):
    ...
    low_stock, _total = service.list_products(skip=0, limit=100, low_stock=True)
