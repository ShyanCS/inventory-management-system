...
def test_create_customer(db_session):
    ...
    create_dto = CustomerCreate(
        full_name="Alice Smith", email="alice@example.com", phone="+1-555-0100"
    )
...
def test_create_customer_duplicate_email(db_session):
    service = CustomerService(db_session)
    create_dto = CustomerCreate(full_name="Bob Jones", email="bob@example.com", phone="555-0101")
...
def test_delete_customer(db_session):
    ...
    created = service.create_customer(
        CustomerCreate(full_name="Del", email="del@test.com", phone="1")
    )
...
def test_delete_customer_with_orders(db_session):
    ...
    customer = c_service.create_customer(
        CustomerCreate(full_name="Del", email="del2@test.com", phone="1")
    )
    product = p_service.create_product(
        ProductCreate(name="P", sku="SKU-DEL", price=10, quantity_in_stock=5)
    )

    o_service.create_order(
        OrderCreate(
            customer_id=customer.id, items=[OrderItemCreate(product_id=product.id, quantity=1)]
        )
    )
