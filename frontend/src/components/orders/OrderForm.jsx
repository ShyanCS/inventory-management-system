...
export default function OrderForm({ customers, products, onSave, onCancel, apiError }) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg rounded-xl glass-panel shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        {/* Header */}
        ...
        {apiError && (
          <div
            role="alert"
            className="mx-6 mt-4 glass-card !bg-rose-500/5 !border-rose-500/30 px-4 py-3 text-sm text-rose-400 flex-shrink-0"
          >
        ...
        <div className="overflow-y-auto flex-1 custom-scrollbar">
          <form onSubmit={handleSubmit} noValidate id="order-form" className="space-y-6 px-6 py-5">
            ...
            <div>
              ...
              {errors.customer_id && (
                <p className="mt-1.5 text-xs text-rose-400">{errors.customer_id}</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {items.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex gap-3 items-start rounded-xl glass-card bg-white/[0.01] p-3"
                  >
                    {/* Product */}
                      ...
                      >
                        ...
                        {products.map((p) => (
                          <option key={p.id} value={p.id} className="bg-slate-900">
                            {p.name} — ${Number(p.price).toFixed(2)} ({p.quantity_in_stock} in
                            stock)
                          </option>
                        ))}
                      </select>
            ...
            {liveTotal > 0 && (
              <div className="flex items-center justify-between rounded-xl glass-card !bg-amber-500/10 !border-amber-500/20 px-5 py-4">
                <span className="text-sm font-medium text-amber-200/70 uppercase tracking-wider">
                  Estimated Total
                </span>
